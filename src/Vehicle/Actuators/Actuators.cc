...
static Mixer::Function actuatorParamFunction(const QString& function, const Parameter& parameter)
{
    const QString functionLower = function.toLower();
    if (functionLower == "posx") {
        return Mixer::Function::PositionX;
    } else if (functionLower == "posy") {
        return Mixer::Function::PositionY;
    } else if (functionLower == "posz") {
        return Mixer::Function::PositionZ;
    } else if (functionLower == "spin-dir") {
        return Mixer::Function::SpinDirection;
    } else if (functionLower == "axisx") {
        return Mixer::Function::AxisX;
    } else if (functionLower == "axisy") {
        return Mixer::Function::AxisY;
    } else if (functionLower == "axisz") {
        return Mixer::Function::AxisZ;
    } else if (functionLower == "type") {
        return Mixer::Function::Type;
    } else if (functionLower == "tilt" || functionLower == "tilt-primary") {
        return Mixer::Function::Tilt;
    } else if (functionLower == "tilt2" || functionLower == "tilt-secondary" || functionLower == "secondary-tilt") {
        return Mixer::Function::SecondaryTilt;
    }

    if (!function.isEmpty()) {
        return Mixer::Function::Unspecified;
    }

    const QString labelLower = parameter.label.toLower();
    const QString nameUpper = parameter.name.toUpper();
    const bool looksSecondaryTilt = labelLower.contains("secondary") || labelLower.contains("second") ||
            labelLower.contains("stage 2") || nameUpper.contains("TILT2") || nameUpper.contains("TILT_2") ||
            nameUpper.contains("TILT_SECOND") || nameUpper.contains("TILT_SEC");
    if (looksSecondaryTilt) {
        return Mixer::Function::SecondaryTilt;
    }

    if (labelLower.contains("tilted by") || (labelLower.contains("tilt") && nameUpper.contains("_TILT"))) {
        return Mixer::Function::Tilt;
    }
    ...
    for (const auto&& mixerConfigJsonValue : mixerConfigJsonArr) {
        ...
        for (const auto&& actuatorJson : actuatorsJson) {
            ...
            for (const auto&& parameterJson : perItemParametersJson) {
                ...
                const QString function = parameter["function"].toString();
                mixerParameter.function = actuatorParamFunction(function, mixerParameter.param);
                if (!function.isEmpty() && mixerParameter.function == Mixer::Function::Unspecified) {
