...
static QStringList secondaryTiltParamCandidates(const QString& primaryParam)
{
    ...
    const QString upperPrimary = primaryParam.toUpper();
    if (upperPrimary.endsWith("_TILT")) {
        const QString prefix = primaryParam.left(primaryParam.size() - 5);
        addCandidate(prefix + "_TILT2");
        addCandidate(prefix + "_TILT_2");
        addCandidate(prefix + "_TILT_SECOND");
        addCandidate(prefix + "_TILT_SEC");
    }

    const int tiltIndex = upperPrimary.lastIndexOf("TILT");
    if (tiltIndex >= 0) {
        addCandidate(primaryParam.left(tiltIndex) + "TILT2" + primaryParam.mid(tiltIndex + 4));
        addCandidate(primaryParam.left(tiltIndex) + "TILT_2" + primaryParam.mid(tiltIndex + 4));
        addCandidate(primaryParam.left(tiltIndex) + "TILT_SECOND" + primaryParam.mid(tiltIndex + 4));
    }

    addCandidate(primaryParam + "2");
    addCandidate(primaryParam + "_2");
    addCandidate(primaryParam + "_SEC");
    addCandidate(primaryParam + "_SECOND");
    addCandidate(primaryParam + "_STAGE2");
...
ChannelConfigInstance* ChannelConfigSecondaryTilt::instantiate(int paramIndex, int actuatorTypeIndex,
        ParameterManager* parameterManager, std::function<void(Function, Fact*)> factAddedCb)
{
    QString primaryParam = config().param.name;
    const int usedParamIndex = isActuatorTypeConfig() ? actuatorTypeIndex + indexOffset() : paramIndex + indexOffset();
    primaryParam.replace("${i}", QString::number(usedParamIndex));

    Fact* fact = nullptr;
    for (const QString& param : secondaryTiltParamCandidates(primaryParam)) {
    ...
    if (!fact) {
        int tiltCount = 0;
        if (parameterManager->parameterExists(ParameterManager::defaultComponentId, "CA_SV_TL_COUNT")) {
            tiltCount = parameterManager->getParameter(ParameterManager::defaultComponentId, "CA_SV_TL_COUNT")->rawValue().toInt();
        }
        ChannelConfigInstance* instance = new ChannelConfigInstanceSecondaryTilt(this, *this, tiltCount);
        channelInstanceCreated(instance);
        return instance;
    } else {
        ChannelConfigInstance* instance = new ChannelConfigInstance(this, fact, *this);
...
void ChannelConfigInstanceSecondaryTilt::allInstancesInitialized(QmlObjectListModel* configInstances)
{
    for (int i = 0; i < configInstances->count(); ++i) {
        auto channelConfigInstance = configInstances->value<ChannelConfigInstance*>(i);
        if (channelConfigInstance->channelConfig()->function() == Function::Tilt) {
            _primaryTilt = channelConfigInstance;
            break;
        }
    }

    if (!_primaryTilt || !_primaryTilt->fact()) {
        qCDebug(MixerLog) << "ActuatorOutputChannel: Primary tilt fact missing for secondary tilt";
        return;
    }

    FactMetaData* metaData = new FactMetaData(*_primaryTilt->fact()->metaData(), this);
    metaData->setName("secondary_tilt");
    metaData->setShortDescription(tr("Secondary tilt assignment"));
    metaData->setReadOnly(false);
    _fact = new Fact("", metaData, this);

    setFactFromPrimaryTilt();
    connect(_primaryTilt->fact(), &Fact::rawValueChanged,
            this, &ChannelConfigInstanceSecondaryTilt::setFactFromPrimaryTilt);
}

void ChannelConfigInstanceSecondaryTilt::setFactFromPrimaryTilt()
{
    if (!_primaryTilt || !_primaryTilt->fact() || !_fact) {
        return;
    }

    const int primaryTilt = _primaryTilt->fact()->rawValue().toInt();
    int secondaryTilt = 0;

    const int primaryTiltCount = _tiltCount / 2;
    if (_tiltCount >= 4 && primaryTilt > 0 && primaryTilt <= primaryTiltCount) {
        secondaryTilt = primaryTilt + primaryTiltCount;
    }

    _fact->setRawValue(secondaryTilt);
}
...
Fact* MixerChannel::getFact(Function function) const
{
    for (int i = 0; i < _configInstances->count(); ++i) {
        ChannelConfigInstance* configInstance = _configInstances->value<ChannelConfigInstance*>(i);
        if (configInstance->channelConfig()->function() == function) {
...
ChannelConfigInstance* MixerChannel::secondaryTiltInstance() const
{
    for (int i = 0; i < _configInstances->count(); ++i) {
        ChannelConfigInstance* configInstance = _configInstances->value<ChannelConfigInstance*>(i);
        if (configInstance->channelConfig()->function() == Function::SecondaryTilt) {
...
void Mixers::update()
{
    ...
    if (_selectedMixer != -1) {
        ...
        for (const auto &actuatorGroup : actuatorGroups) {
            ...
            int axisIdx[3]{-1, -1, -1};
            int primaryTiltIdx = -1;
            bool hasSecondaryTilt = false;
            for (const auto& perItemParam : actuatorGroup.perItemParameters) {
                ...
                } else if (perItemParam.function == Function::Tilt && primaryTiltIdx == -1) {
                    primaryTiltIdx = currentMixerGroup->channelConfigs()->count() - 1;
                } else if (perItemParam.function == Function::SecondaryTilt) {
                    hasSecondaryTilt = true;
                }
            ...
            if (primaryTiltIdx >= 0 && !hasSecondaryTilt) {
                ChannelConfig* primaryTiltConfig = currentMixerGroup->channelConfigs()->value<ChannelConfig*>(primaryTiltIdx);
                MixerParameter parameter = primaryTiltConfig->config();
                parameter.function = Function::SecondaryTilt;
                parameter.identifier = "";
                currentMixerGroup->addChannelConfig(new ChannelConfigSecondaryTilt(currentMixerGroup, parameter));
            }
